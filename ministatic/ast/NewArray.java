...
public class NewArray extends Expr {
	...
	//Makes sure every varible used in size is in the current scope
	//expr analyze functions are void, so nothing to catch
	size.analyze(scoping, env);
	...
	//requires that the expr size has an Integer return value
	Type type = size.require(typing, Type.INT);
	return type; 
    }
	...
	//Makes sure everything used in the size expr has been initialized 
	size.analyze(init, initialized);
